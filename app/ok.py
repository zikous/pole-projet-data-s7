...
def main():
    ...
    # Configure simulation parameters with target return of 25%
    config = PortfolioConfig(
        initial_capital=100000,  # Starting with $100,000
        lookback_months=6,  # 6 months of historical data for optimization
        total_months=12 * 3,  # 3 years of simulation
        start_year=2021,  # Start from 2021
        tickers=[
            "AAPL",  # Apple
            "MSFT",  # Microsoft
    ...
    try:
        ...
        for strategy in simulator.weights:
            if strategy == "target_return" and config.target_return is None:
                continue
            final_weights = simulator.weights[strategy][-1]
            print(f"\n{strategy.replace('_', ' ').title()} Strategy:")
            for ticker, weight in zip(config.tickers, final_weights):
